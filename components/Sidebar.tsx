...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onChangeView, isOpen, onClose, draftCount, reviewCount, user, onSignOut }) => {
      ...
      `}>
        ...
        <div className="p-4 border-t border-gray-100 dark:border-gray-700 space-y-2">
          ...
          {onSignOut && (
            <button
              onClick={onSignOut}
              className={`flex items-center gap-2 w-full px-3 py-2 rounded-xl text-xs font-semibold text-gray-500 dark:text-gray-400 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-red-400 transition-colors ${isCollapsed ? 'justify-center' : ''}`}
              title="Sign out"
            >
              <LogOut size={15} />
